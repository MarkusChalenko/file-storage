...
async def upload_file(
    ...
    file_upload = await files_crud.upload_file(
        ...
        user=user,
        path=path)
    logger.info('A new file successfully uploaded.')
...
async def get_files_list(
    ...
    if not result:
        ...
        logger.info('The file list is taken from the DB.')
        return FilesList(account_id=user.id, files=files_list)
    logger.info('The file list is taken from the Redis cache.')
    return result
...
async def files_search(
    ...
    logger.info('Starting a file search.')
    ...
    if not result:
        ...
        logger.info('Searching for files in the DB.')
        return new_result
    logger.info('Searching for files in the Redis cache.')
    return result
